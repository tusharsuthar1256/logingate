...
import { InteractiveChart, DataPoint } from '../../(components)/InteractiveChart';
...
const Dashboard: React.FC<DashboardProps> = ({ onNavigateHome, initialTab = 'analysis' }) => {
...
const AnalysisView = () => {
  ...
  const [stats, setStats] = useState({
    ...
    threatsBlocked: 0,
    avgLatency: 0,
    distribution: [] as { label: string, percent: number, color: string }[],
    chartData: [] as { day: string, requests: number, fullDate: string }[],
    safeData: [] as DataPoint[],
    fakeData: [] as DataPoint[],
    maxChartHeightValue: 100
  });
  ...
  const fetchSummary = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/usage/summary?userId=${user?.id}`);
      if (res.ok) {
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="flex flex-col items-center text-gray-400 gap-4">
          ...
          <p>Loading real-time usage data...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="grid lg:grid-cols-3 gap-6 mt-6 sm:mt-8">
        <div className="lg:col-span-2 p-4 sm:p-6 rounded-2xl sm:rounded-3xl bg-white/5 border border-white/10 shadow-sm">
          ...
          <div className="h-64 flex items-end justify-between gap-4 px-2">
            {stats.chartData.length > 0 ? stats.chartData.map((dataLabel, i) => {
              const heightPercent = stats.maxChartHeightValue > 0 ? (dataLabel.requests / stats.maxChartHeightValue) * 100 : 0;
              return (
                <div key={i} className="w-full bg-white/5 rounded-t-lg relative group h-full flex items-end">
                  <motion.div
                    ...
                    className="w-full absolute bottom-0 bg-primary rounded-t-lg group-hover:bg-primary transition-colors min-h-[4px]"
                  />
                  <div className="absolute -top-10 left-1/2 -translate-x-1/2 bg-gray-900 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10 pointer-events-none">
                    {dataLabel.requests} reqs
                  </div>
                </div>
              );
            }) : (
              <div className="text-gray-400 w-full text-center pb-10">No usage data for the last 7 days.</div>
            )}
          </div>
          <div className="flex justify-between mt-4 text-xs text-gray-400 font-mono">
            {stats.chartData.length > 0 && stats.chartData.map((d, i) => <span key={i}>{d.day}</span>)}
          </div>
        </div>
        ...
        <div className="p-4 sm:p-6 rounded-2xl sm:rounded-3xl bg-white/5 border border-white/10 shadow-sm">
          ...
          <div className="space-y-6">
            {stats.distribution.length > 0 ? stats.distribution.map((threat, idx) => (
              ...
              <div className="text-gray-400 text-center py-6 text-sm">No threats recorded yet.</div>
            )}
          </div>
  ...
  return (
    <div className="max-w-6xl mx-auto min-h-[600px]">
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-6">
        <div className="bg-white dark:bg-white/5 p-1 rounded-xl flex items-center">
          <button
            onClick={() => setListType('whitelist')}
            className={`flex items-center gap-2 px-6 py-2.5 rounded-lg text-sm font-medium transition-all ${listType === 'whitelist'
          ...
          <button
            ...
            className={`flex items-center gap-2 px-6 py-2.5 rounded-lg text-sm font-medium transition-all ${listType === 'blacklist'
                  ...
                  <tr key={item.id} className="hover:bg-white/5 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        <span className="text-white font-mono font-medium text-sm">{item.identity}</span>
                        <span className="text-xs text-gray-500 flex items-center gap-1 mt-0.5">
                          <span className="uppercase text-[10px] border border-white/10 px-1 rounded">{item.type}</span>
                          {item.date}
                        </span>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border ${item.status === 'active'
                        ? listType === 'whitelist'
                          ? 'bg-green-500/10 text-green-400 border-green-500/20'
                          : 'bg-red-500/10 text-red-400 border-red-500/20'
                        : 'bg-white/10 text-gray-400 border-white/10'
                        }`}>
                        <span className={`w-1.5 h-1.5 rounded-full ${item.status === 'active'
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-100 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-lg bg-[#121214] rounded-3xl shadow-2xl overflow-hidden border border-white/10"
            >
              ...
              <form onSubmit={handleAddItem} className="p-6 space-y-4">
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button type="button" className={`py-2 rounded-xl text-sm font-medium border ${listType === 'whitelist' ? 'border-emerald-500 bg-emerald-500/10 text-emerald-600' : 'border-white/10 text-gray-500'}`} onClick={() => setListType('whitelist')}>
                      Whitelist
                    </button>
                    <button type="button" className={`py-2 rounded-xl text-sm font-medium border ${listType === 'blacklist' ? 'border-red-500 bg-red-500/10 text-red-600' : 'border-white/10 text-gray-500'}`} onClick={() => setListType('blacklist')}>
                      Blacklist
                    </button>
                  </div>
                </div>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1.5">Identity</label>
                  <input
                    type="text"
                    placeholder="e.g. user@example.com or 192.168.1.1"
                    className="w-full px-4 py-2.5 rounded-xl bg-black/50 border border-white/10 text-white focus:ring-2 focus:ring-primary outline-none"
                  />
                </div>
                ...
                <div className="pt-4 flex gap-3">
                  ...
                  <button
                    type="submit"
                    className="flex-1 px-4 py-2.5 rounded-xl bg-white text-black font-bold hover:opacity-90 transition-opacity"
                  >
                    Add Rule
                  </button>
                </div>
...
const CodeBlock = ({ children }: { children?: React.ReactNode }) => (
  <div className="my-6 rounded-xl bg-[#161618] border border-white/5 p-4 overflow-x-auto font-mono text-sm text-gray-300">
    {children}
  </div>
);
...
const DocsView = () => {
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-[80vh] relative">
      {/* Left Sidebar (Sticky) */}
      <aside className="hidden lg:block w-64 shrink-0 border-r border-white/10 mr-12 pr-6">
        <div className="sticky top-32">
          <div className="space-y-8">
            <div>
              <h5 className="font-bold text-white mb-3 text-sm tracking-wide">Getting Started</h5>
              <ul className="space-y-1">
                ...
                <li className="text-sm text-gray-400 hover:text-white px-3 py-2 cursor-pointer transition-colors border-l-2 border-transparent hover:border-gray-500">Authentication</li>
                <li className="text-sm text-gray-400 hover:text-white px-3 py-2 cursor-pointer transition-colors border-l-2 border-transparent hover:border-gray-500">Errors</li>
              </ul>
            </div>

            <div>
              <h5 className="font-bold text-white mb-3 text-sm tracking-wide">Resources</h5>
              <ul className="space-y-1">
                <li className="text-sm text-gray-400 hover:text-white px-3 py-2 cursor-pointer transition-colors border-l-2 border-transparent hover:border-gray-500">Rate Limits</li>
                <li className="text-sm text-gray-400 hover:text-white px-3 py-2 cursor-pointer transition-colors border-l-2 border-transparent hover:border-gray-500">Client Libraries</li>
              </ul>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 min-w-0 pb-16">
        ...
        <h2 className="text-2xl font-bold text-white mt-12 mb-4 border-b border-white/10 pb-2">Quick Start</h2>
        <p className="text-gray-400 leading-relaxed mb-4">
          Make a GET request to the <code className="bg-white/10 px-1.5 py-0.5 rounded text-sm font-mono text-primary">/verify</code> endpoint with an email address.
        </p>
  ...
  -H "Authorization: Bearer YOUR_API_KEY"`}</pre>
        ...
        <h2 className="text-2xl font-bold text-white mt-12 mb-4 border-b border-white/10 pb-2">Response Format</h2>
        <p className="text-gray-400 leading-relaxed mb-4">
    ...
    "verdict": `}<span className="text-green-500">"allow"</span>{`
        ...
        <div className="mt-16 pt-8 border-t border-white/10 flex justify-between">
          ...
          <a href="#" className="group flex flex-col items-end gap-1 text-right">
            ...
            <div className="flex items-center gap-2 text-lg font-bold text-white group-hover:text-primary transition-colors">
      ...
      {/* Right Sidebar (TOC) */}
      <aside className="hidden xl:block w-64 shrink-0 ml-12 border-l border-white/10 pl-6">
        <div className="sticky top-32">
          <h5 className="font-bold text-white mb-4 text-xs uppercase tracking-wider">On This Page</h5>
          <ul className="space-y-3 text-sm">
            <li className="text-gray-400 hover:text-white cursor-pointer transition-colors">Quick Start</li>
            <li className="text-gray-400 hover:text-white cursor-pointer transition-colors">Response Format</li>
          </ul>
        </div>
      </aside>
    </div>
      ...
      <div className="grid gap-6">
        <div className="grid md:grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" defaultValue="Alex" className="w-full px-4 py-2 rounded-xl bg-black border border-white/10 text-white focus:ring-2 focus:ring-primary outline-none" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-1">Last Name</label>
            <input type="text" defaultValue="Morgan" className="w-full px-4 py-2 rounded-xl bg-black border border-white/10 text-white focus:ring-2 focus:ring-primary outline-none" />
          </div>
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-300 mb-1">Email Address</label>
          <input type="email" defaultValue="alex@company.com" className="w-full px-4 py-2 rounded-xl bg-black border border-white/10 text-white focus:ring-2 focus:ring-primary outline-none" />
        </div>
      </div>
